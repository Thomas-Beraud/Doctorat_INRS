...
Object_Observation_Weight = Observation_Weight(nx,
                            ny,
                            nz,
                            x_step,
                            y_step,
...
local_index, local_var = clean_ellipse_index(Object_Observation_Weight.local_index_, Object_Observation_Weight.local_var_)
...
To show a 2D section along y = 2
[:,2,:]
To show a 2D section along x = 25
[:,:,25]

for i in range(y_range):
	plt.imshow(local_ellipse_value.reshape(z_range,y_range,x_range)[:,i,:])
	plt.title(str(i+y_min))
	plt.show()
"""
...
plt.subplot(122)
plt.imshow(localisation,aspect='auto')
plt.colorbar()
